...
IT Provisioning Agent — an A2A agent (Mule 4.11, Java 17). Orders and repairs loaner laptops (mock). An employee ID is required for either request.
...
```bash
mvn -B package -DskipTests      # target/it-provisioning-agent-app-1.7.8-mule-application.jar

yc deploy file <org> Sandbox <business-group-id> it-provisioning-agent-app 1.7.8 \
  target/it-provisioning-agent-app-1.7.8-mule-application.jar target=ch2:<cluster> name=it-provisioning-agent \
...
After deploying, set the app's URL as `it-provisioning-agent{{-suffix}}.url` in the network's `exchange.json`.
